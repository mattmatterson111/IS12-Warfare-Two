...
/client/proc/add_admin_verbs()
	if(holder)
		add_verbs(admin_verbs_default)
		if(holder.rights & R_BUILDMODE)		add_verbs(/client/proc/togglebuildmodeself)
		if(holder.rights & R_ADMIN)			add_verbs(admin_verbs_admin)
		if(holder.rights & R_BAN)			add_verbs(admin_verbs_ban)
		if(holder.rights & R_FUN)			add_verbs(admin_verbs_fun)
		if(holder.rights & R_SERVER)		add_verbs(admin_verbs_server)
		if(holder.rights & R_DEBUG)
			add_verbs(admin_verbs_debug)
			if(config.debugparanoid && !(holder.rights & R_ADMIN))
				verbs.Remove(admin_verbs_paranoid_debug)			//Right now it's just callproc but we can easily add others later on.
		if(holder.rights & R_POSSESS)		add_verbs(admin_verbs_possess)
		if(holder.rights & R_PERMISSIONS)	add_verbs(admin_verbs_permissions)
		if(holder.rights & R_STEALTH)		add_verbs(/client/proc/stealth)
		if(holder.rights & R_REJUVINATE)	add_verbs(admin_verbs_rejuv)
		if(holder.rights & R_SOUNDS)		add_verbs(admin_verbs_sounds)
		if(holder.rights & R_SPAWN)			add_verbs(admin_verbs_spawn)
		if(holder.rights & R_MOD)			add_verbs(admin_verbs_mod)
		if(holder.rights & R_MENTOR)		add_verbs(admin_verbs_mentor)
