...
/datum/event/infestation/proc/set_location_get_infestation_turfs()
	location = pick_area(list(/proc/is_not_space_area, GLOBAL_PROC_REF(is_station_area)))
	if(!location)
		log_debug("Vermin infestation failed to find a viable area. Aborting.")
		kill()
		return

	var/list/vermin_turfs = get_area_turfs(location, list(/proc/not_turf_contains_dense_objects, GLOBAL_PROC_REF(IsTurfAtmosSafe)))
	if(!vermin_turfs.len)
