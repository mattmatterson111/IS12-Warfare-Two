...
/turf/simulated/floor/exoplanet/water/shallow/New()
	...
	if(locate(/obj/structure) in src)
		for(var/obj/structure/structure in src)
			if(istype(structure, /obj/structure/landmine))
				qdel(structure)
			else if(istype(structure, /obj/structure/barbwire))
				qdel(structure)
			else if(istype(structure, /obj/structure/dirt_wall))
				qdel(structure)
	if(has_light && !(locate(/obj/effect/lighting_dummy/daylight) in src) && !(locate(/obj/effect/map_entity/environment_blocker) in src))
		new /obj/effect/lighting_dummy/daylight(src)
	//temperature = T0C - 80
